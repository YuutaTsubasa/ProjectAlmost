...
Add `playerActorDefinition.crouch.body` for the crouch pose. The crouch body must be shorter than the standing body and keep the feet grounded by increasing `offsetY` by the removed height. The exact dimensions are owned by the rebuild domain manifest and verified by tests.
