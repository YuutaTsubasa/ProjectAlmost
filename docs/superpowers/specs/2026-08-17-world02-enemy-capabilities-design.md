...
- Seed Lantern must stay readable when a coin overlaps its position. It renders above coins and uses a single idle frame with floating motion from the renderer tween, avoiding the generated sheet's uneven frame-to-frame scale changes. Current presentation uses idle frame `1` instead of frame `0`.
